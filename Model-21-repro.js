...
function cargarTracksDesdeJSON() {
    fetch("https://bydesign-xat.github.io/Music/Model-21-repro10.json")
        .then(res => res.json())
        .then(data => {
            trackData = data;
            currentTrack = 0;
...
function activarReproduccion(index, modo = "manual") {
  ...
  document.querySelectorAll('.modal-track-item')
...
currentTrackName.textContent =
    `${track.artist || "Unknown artist"} - ${track.title || "Untitled"}`;
  if (metaTrack) {
    metaTrack.textContent = `${track.artist || "Unknown artist"} - ${track.title || "Untitled"} | ${track.album || "Álbum desconocido"} | ⏱ ${track.duracion || "--:--"}`;
  }
...
function generarListaModal() {
  ...
  if (modoActual === "radio") {
    ...
    if (currentTrackNameModal) {
      ...
      currentTrackNameModal.textContent = `${artista} — ${titulo}`;
    }
  ...
  if (currentTrackNameModal) {
    if (currentTrack !== null && trackData[currentTrack]) {
      ...
      currentTrackNameModal.textContent = `${t.artist} — ${t.title || "Artista desconocido"}`;
    } else {
  ...
  trackData.forEach((track, index) => {
    const li = document.createElement("li");
    li.textContent = `${index + 1}. ${track.artist} — ${track.title || "Artista desconocido"}`;
...
function actualizarMetadata(track) {
  ...
  const texto = `${track.artist} — ${track.title} | ${track.album} | ⏱ ${track.duracion}`;
...
class Particle {
  constructor() {
    ...
    this.color = `hsl(${210 + Math.random() * 30}, 100%, 50%)`;
  }
